...
class KruskalAlgorithm {
  ...
  // create class Edge to create an edge of the graph that implements Comparable
  // interface
  ...
  void applyKruskal() {
    ...
    int newEdge = 0;
    int index = 0;
    for (index = 0; index < vertices; ++index) finalResult[index] = new Edge();
    Arrays.sort(edgeArray);
    Subset subsetArray[] = new Subset[vertices];
    for (index = 0; index < vertices; ++index) subsetArray[index] = new Subset();
    ...
    for (index = 0; index < newEdge; ++index)
      System.out.println(
          finalResult[index].source
              + " - "
              + finalResult[index].destination
              + ": "
